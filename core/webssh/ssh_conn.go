...
func (ssConn *SshConn) ReceiveWsMsg(wsConn *websocket.Conn, logBuff *bytes.Buffer, exitCh chan bool) {
	...
	for {
		...
		default:
			...
			if err != nil {
				log.Error("reading webSocket message failed",zap.Error(err))
				return
			}
			...
			case wsMsgResize:
				...
				if msgObj.Cols > 0 && msgObj.Rows > 0 {
					if err := ssConn.Session.WindowChange(msgObj.Rows, msgObj.Cols); err != nil {
						log.Error("ssh pty change windows size failed",zap.Error(err))
					}
				}
			case wsMsgCmd:
				...
				decodeBytes := wsData
				if err != nil {
					log.Error("websock cmd string base64 decoding failed",zap.Error(err))
				}
				if _, err := ssConn.StdinPipe.Write(decodeBytes); err != nil {
					log.Error("ws cmd bytes write to ssh.stdin pipe failed",zap.Error(err))
				}
				//write input cmd to log buffer
				if _, err := logBuff.Write(decodeBytes); err != nil {
					log.Error("write received cmd into log buffer failed",zap.Error(err))
				}
			}
		}
	}
	...
	for {
		...
		case <-tick.C:
			...
			if err := flushComboOutput(ssConn.ComboOutput, wsConn); err != nil {
				log.Error("ssh sending combo output to webSocket failed",zap.Error(err))
				return
			}
		case <-exitCh:
			return
...
func (ssConn *SshConn) SessionWait(quitChan chan bool) {
	if err := ssConn.Session.Wait(); err != nil {
		log.Error("ssh session wait failed",zap.Error(err))
		setQuit(quitChan)
	}
}
